...
#               - location of the started executable OR
...
def WriteJsonFile (DialogWindow):
    ...
    data = {}
    data['CommandLine'] = []
    data['CommandLine'].append({
        'autostart':            DialogWindow.cb_a   .GetValue(),
        'PedalStrokeAnalysis':  DialogWindow.cb_A   .GetValue(),
        'ble':                  DialogWindow.cb_b   .GetValue(),
        'CalibrateRR':          DialogWindow.txt_c  .GetValue(),
        #'CtrlCommand':         DialogWindow.na_C   .GetValue(),
        'debug':                DialogWindow.txt_d  .GetValue(),
        'antDeviceID':          DialogWindow.txt_D  .GetValue(),
        'gui':                  DialogWindow.cb_g   .GetValue(),
        'GradeFactor':          DialogWindow.txt_G1 .GetValue(),
        'GradeFactorDH':        DialogWindow.txt_G2 .GetValue(),
        'hrm':                  DialogWindow.txt_H  .GetValue(),
        'manual':               DialogWindow.cb_m   .GetValue(),
        'manualGrade':          DialogWindow.cb_M   .GetValue(),
        'calibrate':        not DialogWindow.cb_n   .GetValue(),
        'factor':               DialogWindow.txt_p  .GetValue(),
        'PowerMode':            DialogWindow.cb_P   .GetValue(),
        'Resistance':           DialogWindow.cb_r   .GetValue(),
        'RunoffMaxSpeed':       DialogWindow.txt_R1 .GetValue(),
        'RunoffDip':            DialogWindow.txt_R2 .GetValue(),
        'RunoffMinSpeed':       DialogWindow.txt_R3 .GetValue(),
        'RunoffTime':           DialogWindow.txt_R4 .GetValue(),
        'RunoffPower':          DialogWindow.txt_R5 .GetValue(),
        'simulate':             DialogWindow.cb_s   .GetValue(),
        #'scs':                 DialogWindow.txt_S  .GetValue(),
        'TacxType':             DialogWindow.combo_t.GetValue(),
        'exportTCX':            DialogWindow.cb_x   .GetValue(),
    })

    if debug.on(debug.Function):
        logfile.Write ('    debug=%s' % (DialogWindow.txt_d.GetValue()))
...
def ReadJsonFile (args):
    ...
    try:
        ...
        try:
            ...
            # Handle content
            # ------------------------------------------------------------------
            for p in data['CommandLine']:
                G = 0
                R = 0
                for q in p:
                    if debug.on(debug.Function):
                        logfile.Write ("%s=%s" % (q, p[q]) )
                    if   q == 'autostart':            args.autostart            = p[q]
                    elif q == 'PedalStrokeAnalysis':  args.PedalStrokeAnalysis  = p[q]
                    elif q == 'ble':                  args.ble                  = p[q]
                    elif q == 'CalibrateRR':          args.CalibrateRR          = p[q]
                    #'CtrlCommand':                   args.CtrlCommand
                    elif q == 'debug':                args.debug                = p[q]
                    elif q == 'antDeviceID':          args.antDeviceID          = p[q]
                    elif q == 'gui':                  args.gui                  = p[q]
                    elif q == 'GradeFactor':          G += 1
                    elif q == 'GradeFactorDH':        G += 1
                    elif q == 'hrm':                  args.hrm                  = p[q]
                    elif q == 'manual':               args.manual               = p[q]
                    elif q == 'manualGrade':          args.manualGrade          = p[q]
                    elif q == 'calibrate':            args.calibrate            = p[q]
                    elif q == 'factor':               args.factor               = p[q]
                    elif q == 'PowerMode':            args.PowerMode            = p[q]
                    elif q == 'Resistance':           args.Resistance           = p[q]
                    elif q == 'RunoffMaxSpeed':       R += 1
                    elif q == 'RunoffDip':            R += 1
                    elif q == 'RunoffMinSpeed':       R += 1
                    elif q == 'RunoffTime':           R += 1
                    elif q == 'RunoffPower':          R += 1
                    elif q == 'simulate':             args.simulate             = p[q]
                    #'scs':                           args.scs                  = p[q]
                    elif q == 'TacxType':             args.TacxType             = p[q]
                    elif q == 'exportTCX':            args.exportTCX            = p[q]
                ...
                # ----------------------------------------------------------------------
                if G == 2:
                    GradeFactor   = p['GradeFactor']
                    GradeFactorDH = p['GradeFactorDH']
                ...
                if R == 5:
                    RunoffMaxSpeed = p['RunoffMaxSpeed']
                    RunoffDip      = p['RunoffDip']
                    RunoffMinSpeed = p['RunoffMinSpeed']
                    RunoffTime     = p['RunoffTime']
                    RunoffPower    = p['RunoffPower']
        ...
        # ----------------------------------------------------------------------
        if debug.on(debug.Function):
            logfile.Console ('    debug=%s' % (args.debug))
            logfile.Write   ("... completed")
...
class dlgFortiusAntSettings(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        l = "Automatically start; “Locate HW” and “Start” if the required devices were found. (-a *)"
        s = (-1, -1)
        ...
        l = "Advertise FortiusAnt as “FortiusAnt Trainer” on a Bluetooth Low Energy dongle. (-b *)"
        s = (-1, -1)
        p = Under(self.cb_a)
        ...
        l = "Run with graphical user interface. (-g *)"
        s = (-1, -1)
        p = Under(self.cb_b)
        ...
        l = "Calibrate the rolling resistance for magnetic brake. (-c)"
        s = (-1, -1)
        ...
        l = "Modify the requested grade with a factor/factorDownhill. (-G)"
        s = (-1, -1)
        ...
        l = "Adjust target Power by multiplying by this factor for static calibration. (-p)"
        s = (-1, -1)
        ...
        l = "Select one specific antDongle (perhaps with a non-standard deviceID). (-D *)"
        s = (-1, -1)
        ...
        l = "Pair this Heart Rate Monitor (0: any, -1: none). Tacx HRM is used if not specified. (-H *)"
        s = (-1, -1)
        p = RightOf(self.txt_H)
        self.lbl_H = wx.StaticText(panel, id=wx.ID_ANY, label=l, pos=p, size=s, style=0, name=wx.StaticTextNameStr)

        l = "Run manual power (ignore target from ANT+ Dongle). (-m)"
        s = (-1, -1)
        p = Under(self.txt_H)
        ...
        l = "Run manual grade (ignore target from ANT+ Dongle). (-M)"
        s = (-1, -1)
        p = Under(self.cb_m)
        self.cb_M = wx.CheckBox(panel, id=wx.ID_ANY, label=l, pos=p, size=s, style=0, validator=wx.DefaultValidator, name=wx.CheckBoxNameStr)
        self.cb_M.Bind(wx.EVT_CHECKBOX, self.EVT_CHECKBOX_cb_M)

        l = "Do not calibrate before start. (-n)"
        s = (-1, -1)
        p = Under(self.cb_M)
        ...
        l = "Power mode has preference over Resistance mode (for 30 seconds). (-P)"
        s = (-1, -1)
        p = Under(self.cb_n)
        ...
        l = "The runoff procedure can be customized: maxSpeed/dip/minSpeed/targetTime/power. (-R)"
        s = (-1, -1)
        p = RightOf(self.txt_R5)
        self.lbl_R = wx.StaticText(panel, id=wx.ID_ANY, label=l, pos=p, size=s, style=0, name=wx.StaticTextNameStr)

        l = "Export TCX file to upload into Strava, Sporttracks, Training peaks. (-x)"
        s = (-1, -1)
        p = Under(self.txt_R1)
        ...
        l = "Create logfile with debugging data. (-d *)"
        s = (-1, -1)
        p = RightOf(self.txt_d)
        self.lbl_d = wx.StaticText(panel, id=wx.ID_ANY, label=l, pos=p, size=s, style=0, name=wx.StaticTextNameStr)
        ...
        l = "Target Resistance = Target Power (to create power curve) (-r)"
        s = (-1, -1)
        p = Under(self.txt_d)
        ...
        l = "Simulate trainer to test ANT+ connectivity. (-s *)"
        s = (-1, -1)
        p = Under(self.cb_r)
